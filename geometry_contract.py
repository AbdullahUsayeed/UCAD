...
--------------------
Hardened geometry helpers that wrap enclosure_builder.py functions.

The AI must call ONLY these functions, never enclosure_builder functions
directly with raw numbers. Every function:

  - Validates inputs against the PrecomputedGeometry object
  - Raises a descriptive ValueError on any out-of-bounds condition
  - Returns the result of the underlying builder call

Usage (in AI-generated code):
    from context_injector import PrecomputedGeometry, precompute
    from geometry_contract import GeometryContract

    def build_enclosure(geo: PrecomputedGeometry):
        c = GeometryContract(geo)
        c.make_base_shell()
        c.add_mounting_boss(0)
        ...
        return c.builder
"""
...
class GeometryContract:
    """
    Validated façade over EnclosureBuilder.

    Every public method:
      - Looks up the pre-computed value from geo
      - Validates inputs
      - Delegates to the underlying EnclosureBuilder
    ...
    def _require_in_range(value: float, lo: float, hi: float, name: str) -> None:
        if not (lo <= value <= hi):
            raise ValueError(
                f"{name}={value:.4f} is outside valid range [{lo:.4f}, {hi:.4f}]"
            )
    ...
    def _require_hole_index(self, index: int) -> dict:
        ...
        if index < 0 or index >= len(holes):
            raise ValueError(
                f"hole_index={index} out of range "
                f"(board has {len(holes)} holes, indices 0–{len(holes) - 1})"
            )
        return holes[index]

    def _check_feature_fits(self, cx: float, cy: float,
    ...
    def make_base_shell(self, wall_t: float | None = None,
                        floor_t: float | None = None):
        """Create the hollow base shell."""
        ...
        if wt > self.geo.enc_outer_width / 2:
            raise ValueError(
                f"wall_t={wt} > half enclosure width "
                f"({self.geo.enc_outer_width / 2:.3f})"
            )
        self.builder.create_base_shell(self._proxy(), wt, ft)
        return self

    def add_mounting_boss(self, hole_index: int,
                          boss_od: float = 6.4, boss_id: float = 3.2,
                          boss_height: float | None = None):
        """Add a mounting boss at holes_enc[hole_index]."""
        ...
        self._check_feature_fits(cx, cy, boss_od / 2, boss_od / 2,
                                 f"boss hole_index={hole_index}")
        ...
        if bh > self.geo.cavity_clearance_mm:
            raise ValueError(
                f"boss_height={bh} exceeds cavity_clearance_mm="
                f"{self.geo.cavity_clearance_mm}"
            )
        self.builder.add_mounting_bosses(self._proxy(), boss_od=boss_od)
        return self

    def add_cutout(self, cx: float, cy: float, wall: str,
                   cut_width: float, cut_height: float,
                   z_centre: float | None = None, clearance: float = 0.5):
        """
        Add a rectangular cutout on `wall` ('left'/'right'/'front'/'back').

        cx, cy — enclosure-space position of cutout centre ON the wall face.
        cut_width, cut_height — opening dimensions in mm.
        """
        ...
        if zc + cut_height / 2 > self.geo.total_height_mm:
            raise ValueError(
                f"Cutout top z={zc + cut_height / 2:.3f} > total_height_mm="
                f"{self.geo.total_height_mm:.3f}"
            )
        self.builder.add_side_cutouts(self.builder.base_shape, [{
            "side": wall, "x": cx, "z": zc - self.geo.floor_t,
            "w": cut_width, "h": cut_height,
        }])
        return self

    def add_cutout_from_hint(self, hint_index: int,
                              clearance: float = 0.5,
                              z_clearance: float = 1.0):
        """Add a cutout from connector_hints[hint_index]."""
        hints = self.geo.connector_hints
        if hint_index < 0 or hint_index >= len(hints):
            raise ValueError(
                f"hint_index={hint_index} out of range "
                f"({len(hints)} hints)"
            )
        ...
        return self.add_cutout(cx=cx, cy=cy, wall=wall,
                               cut_width=cut_w, cut_height=cut_h,
                               z_centre=self.geo.floor_t + cut_h / 2 + z_clearance,
                               clearance=0.0)

    def make_lid(self, lid_t: float = 1.5, lip_depth: float = 1.0):
        """Create flat lid with inset lip."""
        self._require_positive(lid_t, "lid_t")
        self._require_positive(lip_depth, "lip_depth")
        if lip_depth >= self.geo.wall_t:
            raise ValueError(
                f"lip_depth={lip_depth} must be < wall_t={self.geo.wall_t}"
            )
        self.builder.create_lid(self._proxy())
        return self

    def add_snap_fits(self, count: int = 4,
                      snap_width: float = 8.0,
                      snap_depth: float = 3.0,
                      snap_height: float = 2.0):
        """Add cantilever snap hooks."""
        if count < 2:
        ...
        self.builder.add_snap_fits(count=count, snap_width=snap_width,
                                    snap_depth=snap_depth,
                                    snap_height=snap_height)
        return self

    def add_ventilation_slots(self, slot_count: int = 4,
                               slot_width: float = 20.0,
                               slot_height: float = 2.0,
                               slot_spacing: float = 6.0):
        """Add ventilation slots on all walls."""
        if slot_count < 1:
        ...
        if total_band > usable_h:
            raise ValueError(
                f"Vent pattern height={total_band:.2f} > usable wall height="
                f"{usable_h:.2f}"
            )
        self.builder.add_ventilation(slot_count=slot_count,
    ...
    def _proxy(self) -> dict:
        """Build minimal board_data dict from pre-computed geometry."""
        geo = self.geo
        return {
            "dimensions": {
                "x_min": 0.0, "x_max": geo.board_width_mm,
                "y_min": 0.0, "y_max": geo.board_height_mm,
                "width":  geo.board_width_mm,
                "height": geo.board_height_mm,
            },
            "mounting_holes": [
                {"x": h["x"] - geo.board_origin_x,
                 "y": h["y"] - geo.board_origin_y,
                 "diameter": h.get("diameter", 3.2)}
                for h in geo.holes_enc
            ],
            "edge_connectors": [],
            "components": [
                {"x": c["x"] - geo.board_origin_x,
                 "y": c["y"] - geo.board_origin_y,
                 "height": c["height"]}
                for c in geo.components_enc
            ],
        }
...
if __name__ == "__main__":
    ...
    sample = {
        "dimensions": {"x_min": 10, "x_max": 90, "y_min": 5, "y_max": 65,
                        "width": 80, "height": 60},
        "mounting_holes": [
            {"x": 13, "y": 8, "diameter": 3.2},
            {"x": 87, "y": 8, "diameter": 3.2},
            {"x": 13, "y": 62, "diameter": 3.2},
            {"x": 87, "y": 62, "diameter": 3.2},
        ],
        "components": [
            {"ref": "U1", "x": 50, "y": 35, "height": 12},
            {"ref": "J1", "x": 87.5, "y": 35, "height": 8.5,
             "connector": True, "width": 9},
        ],
    }
