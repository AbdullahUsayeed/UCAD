...
class StepState(Enum):
    ...
    @property
    def icon(self):
        return {StepState.PENDING: '⬜', StepState.RUNNING: '⏳', StepState.DONE: '✅', StepState.FAILED: '❌', StepState.CANCELLED: '⊘'}.get(self, '⬜')
    @property
    def label(self):
...
class TaskStep:
    ...
    def to_dict(self):
        return {'id': self.id, 'title': self.title, 'state': self.state.value, 'retries_used': self.retries_used, 'started_at': self.started_at, 'finished_at': self.finished_at, 'summary': self.summary, 'code': self.code, 'error': self.error, 'detail_log': self.detail_log[-50:], 'touched_labels': self.touched_labels}
    def __repr__(self):
