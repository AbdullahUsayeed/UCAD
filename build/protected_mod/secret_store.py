...
def _fernet_key():
    ...
    kdf = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32, salt=b'AICompanion_v1', iterations=200000)
...
def _protect_windows(value, purpose):
    ...
    out_blob = _DataBlob()
    if not crypt32.CryptProtectData(ctypes.byref(data_blob), 'AICompanion', ctypes.byref(entropy_blob), None, None, 1, ctypes.byref(out_blob)):
...
def _unprotect_windows(value, purpose):
    ...
    entropy_blob, entropy_buf = _make_blob(entropy)
    out_blob = _DataBlob()
    if not crypt32.CryptUnprotectData(ctypes.byref(data_blob), None, ctypes.byref(entropy_blob), None, None, 0, ctypes.byref(out_blob)):
        raise ctypes.WinError()
    try:
...
def protect_secret(value, purpose):
    ...
    raise RuntimeError("No secure storage backend available. Install the 'keyring' package (pip install keyring) or 'cryptography' (pip install cryptography) to persist secrets securely on this platform.")
