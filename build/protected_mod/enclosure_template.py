...
import FreeCAD
import Part
CONNECTOR_TYPES: Dict[str, Tuple[float, float]] = {'USB_A': (8.5, 5.0), 'USB_C': (9.0, 3.5), 'USB_MINI': (7.5, 3.0), 'MICRO_USB': (8.0, 2.5), 'HDMI': (15.5, 6.5), 'MINI_HDMI': (11.5, 5.5), 'BARREL_2_5': (8.0, 8.0), 'BARREL_5_5': (10.0, 10.0), 'RJ45': (16.0, 13.5), 'DB9': (25.0, 11.0), 'DB15': (33.4, 11.0), 'SMA': (8.0, 8.0), 'CUSTOM': (0.0, 0.0)}
COLOR_SHELL = (0.85, 0.85, 0.85)
COLOR_LID = (0.75, 0.75, 0.8)
COLOR_PCB = (0.0, 0.6, 0.0)
COLOR_COMPONENT = (0.2, 0.2, 0.2)
COLOR_CONNECTOR = (0.05, 0.05, 0.05)
COLOR_BOSS = (0.85, 0.85, 0.85)
COLOR_HOLE_RING = (0.83, 0.68, 0.21)
COLOR_STANDOFF = (0.3, 0.5, 0.8)
COLOR_SNAP = (0.65, 0.55, 0.1)
@dataclass
class MountingHole:
    x: float
    y: float
...
LID_SCREW_SIZES = {'M2': (3.2, 3.0, 2.2, 4.5, 1.5, 1.2), 'M2.5': (3.8, 3.5, 2.7, 5.0, 1.8, 1.4), 'M3': (4.2, 4.5, 3.2, 6.0, 2.0, 1.8), 'M3.5': (4.8, 5.0, 3.7, 7.0, 2.2, 2.0), 'M4': (5.5, 5.5, 4.3, 8.0, 2.5, 2.2), 'M5': (6.8, 6.5, 5.3, 10.0, 3.0, 2.8), 'M6': (8.0, 7.5, 6.4, 12.0, 3.5, 3.2)}
@dataclass
class EnclosureConfig:
    ...
    def __post_init__(self):
        ...
        if sizes and self.screw_size != 'M3':
            self.lid_insert_od, self.lid_insert_depth, self.lid_screw_clearance_d, self.lid_cbore_d, self.lid_cbore_depth, self.boss_wall = sizes
    connector_clearance: float = 0.3
    connector_z_clearance: float = 0.3
    enable_vents: bool = True
    vent_slot_width: float = 1.5
    vent_slot_gap: float = 2.0
    vent_slot_length: float = 12.0
    vent_wall: str = 'N'
    vent_z_offset: float = 3.0
    enable_label_recess: bool = True
    label_recess_depth: float = 0.6
    label_margin: float = 6.0
    ...
    cable_slot_w: float = 2.5
    custom_cutouts: List[dict] = field(default_factory=list)
    enable_chamfer: bool = True
    enable_lip: bool = True
    enable_connectors: bool = True
    enable_pcb_ref: bool = True
CONNECTOR_KEYWORDS = ['USB', 'HDMI', 'RJ45', 'Audio', 'Terminal', 'Connector', 'Ethernet', 'DSUB', 'SIM', 'SD', 'microSD', 'Header', 'PinHeader', 'Socket', 'Barrel_Jack', 'DC', 'Button', 'SW_', 'Switch']
...
def board_dict_to_boarddata(board_dict: dict):
    ...
    x0 = dims.get('x_min', 0)
    y0 = dims.get('y_min', 0)
    holes = [MountingHole(round(h['x'] - x0, 4), round(h['y'] - y0, 4), h.get('diameter', 2.5)) for h in board_dict.get('mounting_holes', [])]
    edge_connector_refs = {c.get('ref', '') for c in board_dict.get('edge_connectors', []) if c.get('ref')}
    components = []
    for c in board_dict.get('components', []):
        ...
        comp = Component(ref=ref, x=c['x'] - x0, y=c['y'] - y0, height=c.get('height', 3.0), width=c.get('width', 3.0) or 3.0, depth=c.get('length', 3.0) or 3.0, connector=is_conn, connector_type=conn_type, wall=wall, rotation=rotation)
        components.append(comp)
    return BoardData(width=round(dims['width'], 4), height=round(dims['height'], 4), mounting_holes=holes, components=components)
def _clamp(value, lo, hi, name):
    v = float(value)
    if v < lo or v > hi:
        FreeCAD.Console.PrintWarning(f'  [params] {name}={v:.2f} clamped to [{lo:.2f}, {hi:.2f}]\n')
    ...
    if isinstance(custom_cutouts, list):
        ...
        for cc in custom_cutouts:
            ...
            if cc_type not in ('round', 'slot', 'rectangle', 'cable'):
                cc_type = 'round'
            validated.append({'type': cc_type, 'wall': cc.get('wall', 'front'), 'x_mm': float(cc.get('x_mm', 0)), 'y_mm': float(cc.get('y_mm', 0)), 'width_mm': float(cc.get('width_mm', 5)), 'height_mm': float(cc.get('height_mm', cc.get('width_mm', 5))), 'label': str(cc.get('label', ''))})
    else:
        validated = []
    screw_size = params.get('screw_size', 'M3')
    if screw_size not in LID_SCREW_SIZES:
        screw_size = 'M3'
    return EnclosureConfig(wall_thickness=_clamp(params.get('wall_thickness', params.get('wall_t', 3.0)), 0.5, 50.0, 'wall_thickness'), floor_thickness=_clamp(params.get('floor_thickness', params.get('floor_t', 3.0)), 0.5, 25.0, 'floor_thickness'), lid_thickness=_clamp(params.get('lid_thickness', params.get('lid_t', 3.0)), 0.5, 25.0, 'lid_thickness'), margin=_clamp(params.get('margin', 6.0), 0.0, 50.0, 'margin'), screw_size=screw_size, pcb_standoff_height=_clamp(params.get('pcb_standoff_height', 5.0), 0.0, 50.0, 'pcb_standoff_height'), headroom=_clamp(params.get('headroom_mm', params.get('headroom', 10.0)), 0.0, 200.0, 'headroom'), connector_clearance=_clamp(params.get('clearance', params.get('connector_clearance', 0.3)), 0.0, 10.0, 'connector_clearance'), connector_z_clearance=_clamp(params.get('connector_z_clearance', 0.3), 0.0, 10.0, 'connector_z_clearance'), enable_vents=bool(params.get('ventilation', params.get('enable_vents', True))), enable_snaps=bool(params.get('enable_snaps', params.get('snap_fit_count', 0) > 1)), enable_connectors=True, enable_lip=True, enable_pcb_ref=bool(params.get('enable_pcb_ref', params.get('show_pcb', True))), enable_label_recess=bool(params.get('enable_label_recess', True)), vent_slot_width=_clamp(params.get('vent_slot_width', 1.5), 0.5, 25.0, 'vent_slot_width'), custom_cutouts=validated)
...
class Geo:
    @staticmethod
    def box(sx: float, sy: float, sz: float, x: float=0.0, y: float=0.0, z: float=0.0) -> Optional[Part.Shape]:
        ...
        try:
            ...
            return None
    @staticmethod
    def cyl(r: float, h: float, x: float, y: float, z: float) -> Optional[Part.Shape]:
        if r <= 0 or h <= 0:
        ...
        try:
            ...
            return None
    @staticmethod
    def fuse(a: Optional[Part.Shape], b: Optional[Part.Shape]) -> Optional[Part.Shape]:
        if a is None:
            return b
        if b is None:
            return a
        try:
            ...
            return a
    @staticmethod
    def cut(base: Optional[Part.Shape], tool: Optional[Part.Shape]) -> Optional[Part.Shape]:
        ...
        try:
            ...
            return base
    @staticmethod
    def validate_boolean_result(original: Optional[Part.Shape], result: Optional[Part.Shape], label: str) -> bool:
        ...
        if original is not None and (not original.isNull()):
            try:
                ...
                if result.Volume + eps < original.Volume:
                    ...
                    if ratio < 0.1:
                        FreeCAD.Console.PrintError(f'  [geo] BOOLEAN FAILURE — {label} destroyed shape: volume dropped from {original.Volume:.0f} to {result.Volume:.0f} ({ratio * 100:.0f}% remaining). Keeping original.\n')
                        return False
            except Exception:
                pass
        return True
    @staticmethod
    def chamfer_top(shape: Optional[Part.Shape], size: float) -> Optional[Part.Shape]:
        if shape is None or size <= 0:
            return shape
        try:
            bb = shape.BoundBox
            top_z = bb.ZMax
            edges = [e for e in shape.Edges if abs(e.Vertexes[0].Point.z - top_z) < 0.1 and abs(e.Vertexes[1].Point.z - top_z) < 0.1 and (e.Length > size * 2)]
            if edges:
...
def _validate_enclosure_inputs(board: BoardData, config: EnclosureConfig) -> List[str]:
    ...
    for i, sh in enumerate(board.mounting_holes):
        ...
        if not (0 <= sx <= board.width and 0 <= sy <= board.height):
            errors.append(f'Mounting hole #{i} at ({sx:.1f}, {sy:.1f}) is outside PCB bounds (0, 0) – ({board.width:.1f}, {board.height:.1f})')
    for i, comp in enumerate(board.components):
        if not (0 <= comp.x <= board.width and 0 <= comp.y <= board.height):
            errors.append(f'Component {comp.ref} at ({comp.x:.1f}, {comp.y:.1f}) is outside PCB bounds (0, 0) – ({board.width:.1f}, {board.height:.1f})')
    return errors
class EnclosureGeometry:
    def __init__(self, board: BoardData, cfg: EnclosureConfig):
        ...
        if validation_errors:
            FreeCAD.Console.PrintWarning('  ── Input Validation ──\n' + '\n'.join((f'    ⚠ {e}' for e in validation_errors)) + '\n  ─────────────────────\n')
        self._derive()
        self._overlap_standoffs: Set[int] = set()
        self._slot_adjustments: Dict[str, float] = {}
    ...
    def _derive(self):
        ...
        br = self.boss_r
        wt = cfg.wall_thickness
        self.boss_locs = [(wt + br, wt + br), (self.outer_x - wt - br, wt + br), (wt + br, self.outer_y - wt - br), (self.outer_x - wt - br, self.outer_y - wt - br)]
        self.standoff_locs = [(h.x + self.pcb_ox, h.y + self.pcb_oy) for h in b.mounting_holes]
    ...
    def _validate_collisions(self):
        ...
        ok = True
        all_cols = [(bx, by, self.boss_r, 'Boss', idx) for idx, (bx, by) in enumerate(self.boss_locs)] + [(sx, sy, self.standoff_r, 'Standoff', idx) for idx, (sx, sy) in enumerate(self.standoff_locs)]
        n = len(all_cols)
        for i in range(n):
            ...
            for j in range(i + 1, n):
                ...
                min_d = ri + rj + 0.5
                if dist < min_d:
                    FreeCAD.Console.PrintWarning(f'  ⚠ {li} {idi} and {lj} {idj} overlap by {min_d - dist:.2f} mm!\n')
                    ok = False
        ...
        for comp in self.b.components:
            ...
            for idx, (bx, by) in enumerate(self.boss_locs):
                ...
                if cd < cr + self.boss_r:
                    FreeCAD.Console.PrintWarning(f'  ⚠ Connector {comp.ref} overlaps Boss {idx} by {cr + self.boss_r - cd:.2f} mm\n')
                    ok = False
            for idx, (sx, sy) in enumerate(self.standoff_locs):
                cd = math.hypot(cx - sx, cy - sy)
                if cd < cr + self.standoff_r:
                    FreeCAD.Console.PrintWarning(f'  ⚠ Connector {comp.ref} overlaps Standoff {idx} by {cr + self.standoff_r - cd:.2f} mm\n')
                    ok = False
        if self.cfg.enable_vents:
            ...
            for comp in self.b.components:
                ...
                if wall == vw or wall == {'N': 'S', 'S': 'N', 'E': 'W', 'W': 'E'}.get(vw):
                    FreeCAD.Console.PrintWarning(f'  ⚠ Vent wall {vw} may conflict with connector {comp.ref} (on {wall}) — vent slot may be partially occluded\n')
        for ci, cc in enumerate(self.cfg.custom_cutouts):
            ...
            for idx, (bx, by) in enumerate(self.boss_locs):
                ...
                if cd < cr + self.boss_r:
                    FreeCAD.Console.PrintWarning(f'  ⚠ Custom cutout #{ci} (wall={wall}) overlaps Boss {idx} by {cr + self.boss_r - cd:.2f} mm\n')
                    ok = False
        if ok:
            FreeCAD.Console.PrintMessage('  ✓ Collision check passed — all features clear.\n')
        if self._overlap_standoffs:
            FreeCAD.Console.PrintWarning(f'  → {len(self._overlap_standoffs)} standoff(s) overlap a lid boss — will be absorbed into the boss column.\n')
    def _resolve_collisions(self):
        ...
        for comp in self.b.components:
            ...
            min_acceptable = nominal_w + self.cfg.connector_clearance
            overlap_max = 0.0
            for idx, (bx, by) in enumerate(self.boss_locs):
                cd = math.hypot(cx - bx, cy - by)
                ...
                if overlap > 0:
                    ...
                    if actual_reduction > 0:
                        self._slot_adjustments[comp.ref] = max(self._slot_adjustments.get(comp.ref, 0), actual_reduction)
                        resolutions.append(f'    Connector {comp.ref} overlaps Boss {idx} by {overlap:.1f}mm — reduced slot width by {actual_reduction:.1f}mm per side (min {min_acceptable:.1f}mm)\n')
                    else:
                        resolutions.append(f'    Connector {comp.ref} overlaps Boss {idx} by {overlap:.1f}mm — cannot reduce further (slot at minimum {min_acceptable:.1f}mm)\n')
        for comp in self.b.components:
            if not comp.connector:
                continue
            cx = comp.x + self.pcb_ox
            ...
            for idx, (sx, sy) in enumerate(self.standoff_locs):
                ...
                if overlap > 0 and idx not in self._overlap_standoffs:
                    ...
                    resolutions.append(f'    Connector {comp.ref} overlaps Standoff {idx} by {overlap:.1f}mm — standoff absorbed\n')
        if resolutions:
            FreeCAD.Console.PrintMessage('  ── Collision Resolutions ──\n' + ''.join(resolutions) + '  ───────────────────────────\n')
        else:
            FreeCAD.Console.PrintMessage('  ✓ No collisions to resolve.\n')
    def _print_summary(self):
        cfg = self.cfg
        FreeCAD.Console.PrintMessage('\n' + '═' * 60 + f"\n  Outer envelope : {self.outer_x:.2f} × {self.outer_y:.2f} × {self.base_h:.2f} mm\n  Inner cavity   : {self.inner_x:.2f} × {self.inner_y:.2f} × {self.inner_z:.2f} mm\n  PCB origin     : ({self.pcb_ox:.2f}, {self.pcb_oy:.2f}, {self.pcb_z:.2f})\n  Boss radius    : {self.boss_r:.2f} mm   Standoff radius: {self.standoff_r:.2f} mm\n  Vents          : {('ON' if cfg.enable_vents else 'OFF')}   Label recess: {('ON' if cfg.enable_label_recess else 'OFF')}   Snaps: {('ON' if cfg.enable_snaps else 'OFF')}\n" + '═' * 60 + '\n')
    def shell(self) -> Optional[Part.Shape]:
        ...
        cavity = Geo.box(self.inner_x + TOL, self.inner_y + TOL, self.inner_z + TOL, wt - TOL / 2, wt - TOL / 2, ft - TOL / 2)
    ...
    def cut_lip_groove(self, base: Optional[Part.Shape]) -> Optional[Part.Shape]:
        ...
        go = Geo.box(self.outer_x - 2 * lw + TOL, self.outer_y - 2 * lw + TOL, ld + TOL * 2, lw - TOL / 2, lw - TOL / 2, gz - TOL)
        gi = Geo.box(self.inner_x + 2 * lw - TOL, self.inner_y + 2 * lw - TOL, ld + TOL * 2, wt - lw - TOL / 2, wt - lw - TOL / 2, gz - TOL)
    ...
    def cut_connectors(self, base: Optional[Part.Shape]) -> Optional[Part.Shape]:
        ...
        if failures:
            FreeCAD.Console.PrintWarning(f"    {len(failures)} connector cutout(s) failed: {', '.join(failures)}\n")
        return base
    ...
    def lid(self) -> Optional[Part.Shape]:
        ...
        if cfg.enable_lip:
            to_in = tol
            t_outer = Geo.box(self.outer_x - 2 * to_in, self.outer_y - 2 * to_in, ld, to_in, to_in, lid_z - ld)
            ti_in = wt - lw + tol
        ...
        if cfg.enable_label_recess:
            ...
            top = lid_z + lt - rd
            recess = Geo.box(self.outer_x - 2 * lm, self.outer_y - 2 * lm, rd + TOL, lm, lm, top)
    ...
    def snap_arms(self) -> Optional[Part.Shape]:
        ...
        for y_pos, dir_y in [(wt, 1), (self.outer_y - wt, -1)]:
    ...
    def make_cavity(self) -> Optional[Part.Shape]:
        cfg = self.cfg
        return Geo.box(self.inner_x + TOL, self.inner_y + TOL, self.inner_z + TOL, cfg.wall_thickness - TOL / 2, cfg.wall_thickness - TOL / 2, cfg.floor_thickness - TOL / 2)
    ...
    def make_lip_groove_outer(self) -> Optional[Part.Shape]:
        ...
        ld = cfg.lip_depth
        gz = self.base_h - ld + TOL
        return Geo.box(self.outer_x - 2 * lw + TOL, self.outer_y - 2 * lw + TOL, ld + TOL * 2, lw - TOL / 2, lw - TOL / 2, gz - TOL)
    def make_lip_groove_inner(self) -> Optional[Part.Shape]:
        ...
        ld = cfg.lip_depth
        gz = self.base_h - ld + TOL
        return Geo.box(self.inner_x + 2 * lw - TOL, self.inner_y + 2 * lw - TOL, ld + TOL * 2, cfg.wall_thickness - lw - TOL / 2, cfg.wall_thickness - lw - TOL / 2, gz - TOL)
    def make_connector_slot(self, comp) -> Optional[Part.Shape]:
        ...
        pw = max(pw - 2 * reduction, comp.width)
        cx = comp.x + self.pcb_ox
        cy = comp.y + self.pcb_oy
    ...
    def make_custom_cutout(self, cc: dict) -> Optional[Part.Shape]:
        ...
        ez = cfg.floor_thickness + cfg.pcb_standoff_height + PCB_THICKNESS
        z_max = self.base_h - cfg.lip_depth - 1.0
        label = cc.get('label', f'cutout_{wall}')
        def _log_clamp(name, orig, clamped, bound_name, lo, hi):
            if abs(orig - clamped) > 0.01:
                FreeCAD.Console.PrintWarning(f"  ⚠ Cutout '{label}' {name} clamped from {orig:.1f} to {clamped:.1f} ({bound_name} bounds [{lo:.1f}, {hi:.1f}])\n")
        if wall == 'front':
            x0 = max(cx - w / 2, 0.5)
            _log_clamp('x_start', cx - w / 2, x0, 'wall X', 0.5, ox - 0.5)
            x1 = min(cx + w / 2, ox - 0.5)
            _log_clamp('x_end', cx + w / 2, x1, 'wall X', 0.5, ox - 0.5)
            y0 = -TOL
            z0 = max(cy - h / 2, cfg.floor_thickness + 0.5)
            _log_clamp('z_bot', cy - h / 2, z0, 'floor Z', cfg.floor_thickness + 0.5, z_max)
            z1 = min(cy + h / 2, z_max)
            _log_clamp('z_top', cy + h / 2, z1, 'ceil Z', cfg.floor_thickness + 0.5, z_max)
            if x1 <= x0 or z1 <= z0:
            ...
            x0 = -TOL
            y0 = max(cx - w / 2, 0.5)
            _log_clamp('y_start', cx - w / 2, y0, 'wall Y', 0.5, oy - 0.5)
            y1 = min(cx + w / 2, oy - 0.5)
            _log_clamp('y_end', cx + w / 2, y1, 'wall Y', 0.5, oy - 0.5)
            z0 = max(cy - h / 2, cfg.floor_thickness + 0.5)
            _log_clamp('z_bot', cy - h / 2, z0, 'floor Z', cfg.floor_thickness + 0.5, z_max)
            z1 = min(cy + h / 2, z_max)
            _log_clamp('z_top', cy + h / 2, z1, 'ceil Z', cfg.floor_thickness + 0.5, z_max)
            ...
            lz = self.base_h - TOL
            x0 = max(cx - w / 2, 0.5)
            _log_clamp('x_start', cx - w / 2, x0, 'lid X', 0.5, ox - 0.5)
            x1 = min(cx + w / 2, ox - 0.5)
            _log_clamp('x_end', cx + w / 2, x1, 'lid X', 0.5, ox - 0.5)
            y0 = max(cy - h / 2, 0.5)
            _log_clamp('y_start', cy - h / 2, y0, 'lid Y', 0.5, oy - 0.5)
            y1 = min(cy + h / 2, oy - 0.5)
            _log_clamp('y_end', cy + h / 2, y1, 'lid Y', 0.5, oy - 0.5)
            if x1 <= x0 or y1 <= y0:
...
class DocumentBuilder:
    ...
    def _set_color(obj, color_tuple, transparency=0):
        try:
            ...
            pass
    def add(self, shape: Optional[Part.Shape], name: str, color: Tuple[float, float, float], transparency: int=0, z_offset: float=0.0) -> bool:
    ...
    @staticmethod
    def _build_single_solid(positives: List[Optional[Part.Shape]], negatives: List[Optional[Part.Shape]]) -> Optional[Part.Shape]:
        current = None
    ...
    def build_enclosure(self, doc, board: BoardData, config: EnclosureConfig) -> dict:
        ...
        FreeCAD.Console.PrintMessage(f'  Board: {board.width:.1f} x {board.height:.1f} mm  Mounting holes: {len(board.mounting_holes)}  Components: {len(board.components)}  Connectors: {connector_count}\n')
        FreeCAD.Console.PrintMessage(f"  Features: vents={('ON' if config.enable_vents else 'OFF')}  snaps={('ON' if config.enable_snaps else 'OFF')}  lip={('ON' if config.enable_lip else 'OFF')}  cable anchors={('ON' if config.enable_cable_anchors else 'OFF')}\n")
        ...
        shell_positives: List[Optional[Part.Shape]] = [geo.make_outer_box()]
        ...
        shell_negatives: List[Optional[Part.Shape]] = [geo.make_cavity()]
        ...
        lid_positives: List[Optional[Part.Shape]] = [geo.make_lid_plate()]
        ...
        FreeCAD.Console.PrintMessage('\n' + '═' * 60 + f"\n  BILL OF MATERIALS SUMMARY\n  Enclosure outer: {geo.outer_x:.2f} × {geo.outer_y:.2f} × {geo.base_h:.2f} mm\n  {config.screw_size} heat-set inserts × {len(geo.boss_locs)}\n  PCB standoff screws × {len(geo.standoff_locs)}\n  Connector cutouts   × {len(connectors)} ({', '.join((c.connector_type for c in connectors))})\n  Vent slots          : {('Yes (generated)' if config.enable_vents else 'No')}\n  Lid label recess    : {('Yes' if config.enable_label_recess else 'No')}\n  Snap-fit lid arms   : {('Yes (generated)' if config.enable_snaps else 'No')}\n  Cable anchor posts  : {('Yes (generated)' if config.enable_cable_anchors else 'No')}\n" + '═' * 60 + '\n')
        FreeCAD.Console.PrintMessage('  ✓ Build complete.\n\n')
        return {'shell': shell_obj, 'lid': lid_obj, 'compound': compound, 'root': root}
def build_enclosure(board: BoardData, cfg: EnclosureConfig):
    doc = FreeCAD.ActiveDocument
    if doc is None:
        doc = FreeCAD.newDocument('Enclosure_v2')
    enclosure_labels = [obj.Label for obj in doc.Objects if any((kw in obj.Label for kw in ('Enclosure', 'Shell', 'Lid', 'PCB', 'Snap', 'Export')))]
...
if __name__ == '__main__':
    sample_dict = {'dimensions': {'x_min': 10.0, 'x_max': 90.0, 'y_min': 5.0, 'y_max': 65.0, 'width': 80.0, 'height': 60.0}, 'mounting_holes': [{'x': 13.0, 'y': 8.0, 'diameter': 3.2}, {'x': 87.0, 'y': 8.0, 'diameter': 3.2}, {'x': 13.0, 'y': 62.0, 'diameter': 3.2}, {'x': 87.0, 'y': 62.0, 'diameter': 3.2}], 'components': [{'ref': 'U1', 'name': 'QFP-44', 'x': 50.0, 'y': 35.0, 'height': 12.0, 'width': 12.0, 'length': 12.0, 'near_edge': False}, {'ref': 'J1', 'name': 'USB_C_Receptacle', 'x': 87.5, 'y': 35.0, 'height': 3.5, 'width': 9.0, 'length': 5.0, 'near_edge': True, 'connector_width': 9.0}], 'edge_connectors': [{'ref': 'J1', 'name': 'USB_C_Receptacle', 'x': 87.5, 'y': 35.0, 'height': 3.5}]}
