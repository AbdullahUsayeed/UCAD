...
_UNIT_TABLE: Dict[int, Tuple[str, float]] = {0: ('unknown', 1.0), 1: ('inches', 25.4), 2: ('feet', 304.8), 3: ('miles', 1609344.0), 4: ('millimeters', 1.0), 5: ('centimeters', 10.0), 6: ('meters', 1000.0), 7: ('kilometers', 1000000.0), 8: ('microinches', 2.54e-05), 9: ('mils', 0.0254), 10: ('yards', 914.4), 11: ('angstroms', 1e-07), 12: ('nanometers', 1e-06), 13: ('microns', 0.001), 14: ('decimeters', 100.0), 15: ('decameters', 10000.0), 16: ('hectometers', 100000.0), 17: ('gigameters', 1000000000000.0)}
...
def process_dxf(filepath: str, tolerance: Optional[float]=None, layers: Optional[List[str]]=None, merge_multipolygons: bool=True, auto_close: str='off') -> Dict[str, Any]:
    ...
    if doc is None:
        return {'status': 'error', 'error': 'Cannot read DXF file', 'profiles': [], 'warnings': ['Cannot read DXF file'], 'metadata': {}}
    msp = doc.modelspace()
    layer_filter: Optional[Set[str]] = set(layers) if layers else None
    all_xs, all_ys = ([], [])
    raw_entries: List[Tuple[Polygon, str]] = []
    open_entities: List[Tuple[Any, List[tuple], str]] = []
    for entity in msp.query('LWPOLYLINE POLYLINE CIRCLE ARC ELLIPSE SPLINE'):
        try:
            ...
            try:
                poly = Polygon(coords)
                if poly.is_empty or (poly.is_valid and poly.area < 1e-08):
                    continue
    ...
    if all_xs and all_ys:
        extents_max = max(abs(max(all_xs)), abs(min(all_xs)), abs(max(all_ys)), abs(min(all_ys)))
        ...
        for i, (entity, coords, layer) in enumerate(open_entities):
            ...
            if closed_coords is not None:
                ...
                try:
                    ...
                    if not poly.is_empty and poly.area >= 1e-08:
                        raw_entries.append((poly, layer))
                except Exception as e:
                    warn_counts[f'Polygon from auto-closed entity failed: {e}'] += 1
        if auto_close == 'aggressive':
            remaining = [(coords, layer) for i, (e, coords, layer) in enumerate(open_entities) if i not in closed_indices]
            if remaining:
        ...
        try:
            ...
            try:
                for polygon in geo.from_hatch_boundary_path(entity):
                    if polygon and (not polygon.is_empty) and (polygon.area > 1e-08):
    ...
    for geom, _layer in raw_entries:
        try:
            ...
            pass
    if xs_all:
        bbox_diag = math.sqrt((max(xs_all) - min(xs_all)) ** 2 + (max(ys_all) - min(ys_all)) ** 2)
    else:
        bbox_diag = 0.0
    ...
    result = {'status': 'ok', 'profiles': cleaned_profiles, 'warnings': warnings_list, 'units': unit_name, 'unit_scale': mm_factor, 'metadata': {'units': unit_name, 'mm_factor': mm_factor, 'layers': sorted(seen_layers), 'profile_count': len(cleaned_profiles), 'bbox': bbox}}
...
def _normalize_to_origin(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for p in profiles:
        ...
        if bbox and len(bbox) == 4:
            p['bbox'] = [round(bbox[0] - cx, 3), round(bbox[1] - cy, 3), round(bbox[2] - cx, 3), round(bbox[3] - cy, 3)]
    meta = result.get('metadata', {})
    old_bbox = meta.get('bbox')
    if old_bbox and len(old_bbox) == 4:
        meta['bbox'] = [round(old_bbox[0] - cx, 3), round(old_bbox[1] - cy, 3), round(old_bbox[2] - cx, 3), round(old_bbox[3] - cy, 3)]
...
def _scale_polygon(geom, factor: float) -> Polygon:
    ...
    holes = [[(x * factor, y * factor) for x, y in ring.coords] for ring in geom.interiors]
    try:
...
def _classify_profile(layer: str, profile: Dict) -> str:
    ...
    if lu in ('KEEP_OUT', 'KEEPOUT', 'RESTRICT', 'RESTRICTED', 'KEEPOUT_ZONE'):
        return 'keep_out'
    if re.search('\\b(OUTLINE|OUTER|PROFILE)\\b', lu):
        return 'outline'
    if re.search('\\b(CUTOUT?|POCKET|CAVITY|THRU)\\b', lu):
        return 'cutout'
    if re.search('\\b(HOLE|DRILL|THRU)\\b', lu):
        return 'drill'
    if re.search('\\bSLOT\\b', lu):
        return 'slot'
    if re.search('\\b(BEND|FOLD|BEND_LINE)\\b', lu):
        return 'bend_line'
    if re.search('\\b(ENGRAVE|TEXT|MARK|ETCH)\\b', lu):
        return 'engrave'
    if re.search('\\b(KEEP.?OUT|RESTRICT)\\b', lu):
        return 'keep_out'
...
def _is_closed(entity) -> bool:
    try:
        ...
        if entity.dxftype() == 'ELLIPSE':
            ...
            return abs(end - start - 2 * math.pi) < 1e-06
    except Exception:
        return False
    return False
...
def _arc_to_polygon(entity, force_close: bool=False) -> Optional[Polygon]:
    try:
        ...
        is_full = abs(sweep - 360.0) < 1e-06 or abs(sweep) < 1e-06
        if is_full:
            n = 64
            pts = [(cx + r * math.cos(2 * math.pi * i / n), cy + r * math.sin(2 * math.pi * i / n)) for i in range(n)]
            return Polygon(pts)
        ...
        return None
def _try_close_entity(entity, coords: List[tuple], layer: str, tolerance: Optional[float], warn_counts: Counter) -> Optional[List[tuple]]:
    ...
    perimeter = sum((math.sqrt((coords[k + 1][0] - coords[k][0]) ** 2 + (coords[k + 1][1] - coords[k][1]) ** 2) for k in range(len(coords) - 1)))
...
def _sample_entity(entity, samples=20):
    try:
        if entity.dxftype() == 'SPLINE':
            bspline = entity.construction_tool()
            return [(float(bspline.point(t / (samples - 1)).x), float(bspline.point(t / (samples - 1)).y)) for t in range(samples)]
        coords = _entity_coords_path(entity)
        if coords is None:
            coords = _entity_coords_direct(entity)
...
def _chain_open_entities(entries, tolerance, mm_factor, warn_counts):
    ...
    for i, (coords, layer) in enumerate(entries):
        pts = coords or []
        if len(pts) >= 2:
            segments.setdefault(layer, []).append({'start': pts[0], 'end': pts[-1], 'pts': pts, 'idx': i})
    chained = []
...
def _chain_entities_greedy(entries, tolerance, warn_counts):
        ...
        while changed:
            ...
            for i in range(len(pool)):
                ...
                for j, seg in enumerate(pool):
                    ...
                    candidates = [(d_end_to_start, 0, False), (d_end_to_end, 0, True), (d_start_to_start, 1, True), (d_start_to_end, 1, False)]
    ...
    return result
_AREA_CHANGE_THRESHOLD = 0.1
def _process_one_geometry(geom: Polygon, tolerance: float, merge_multipolygons: bool=True) -> List[Dict[str, Any]]:
    ...
    if not geom.is_valid:
        ...
        if orig_area > 0 and area_after > 0:
            ...
            if change > _AREA_CHANGE_THRESHOLD:
                profile_warnings.append(f'self-intersection repair changed area by {change:.1%}')
            else:
    ...
    return [results] if results else []
def _process_profile(geom, tolerance: float, profile_warnings: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if pre_simplify_area > 0 and simplified.area > 0:
        ...
        if change > _AREA_CHANGE_THRESHOLD:
            profile_warnings.append(f'simplification changed area by {change:.1%}')
    geom = simplified
...
def _dedup_consecutive(points: list, tolerance: float=1e-09) -> list:
    if not points:
        return points
    result = [points[0]]
    for pt in points[1:]:
...
def _geom_to_dict(geom, extra_warnings: List[str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {'type': 'Polygon', 'coordinates': [], 'holes': [], 'area': 0, 'bbox': [0, 0, 0, 0], 'profile_type': 'unknown'}
    exterior = _dedup_consecutive(exterior)
    if len(exterior) < 3:
        return None
    holes = [_dedup_consecutive(h) for h in holes if len(h) >= 3]
    min_x, min_y, max_x, max_y = geom.bounds
    return {'type': 'Polygon', 'coordinates': exterior, 'holes': holes, 'area': round(geom.area, 3), 'bbox': [round(min_x, 3), round(min_y, 3), round(max_x, 3), round(max_y, 3)]}
def _detect_overlaps(profiles: List[Dict[str, Any]]) -> List[str]:
    ...
    for orig_i, ga in valid_pairs:
        for jj in tree.query(ga):
            ...
            try:
                if ga.equals(gb):
                    warnings.append(f"Duplicate: layer '{profiles[orig_i]['layer']}' == '{profiles[j]['layer']}'")
                    continue
                ...
                if inter.is_empty:
                    ga_bounds = ga.bounds
                    gb_bounds = gb.bounds
                    min_dim = min(ga_bounds[2] - ga_bounds[0], ga_bounds[3] - ga_bounds[1], gb_bounds[2] - gb_bounds[0], gb_bounds[3] - gb_bounds[1])
                    eps = max(min_dim * 1e-05, 0.001)
                    buf_a = ga.buffer(eps)
                    buf_b = gb.buffer(eps)
                    if not buf_a.intersection(buf_b).is_empty:
                        warnings.append(f"Near-miss: layer '{profiles[orig_i]['layer']}' and '{profiles[j]['layer']}' are close but don't overlap")
                    continue
                union = ga.union(gb)
                if union.is_empty:
                    continue
                ...
                if jaccard > 0.8:
                    warnings.append(f"Near-duplicate: layer '{profiles[orig_i]['layer']}' ~ '{profiles[j]['layer']}' (Jaccard={jaccard:.0%})")
                elif overlap_ratio > 0.01:
                    warnings.append(f"Partial overlap: layer '{profiles[orig_i]['layer']}' and '{profiles[j]['layer']}' overlap by {inter.area:.1f} sq-units (ratio={overlap_ratio:.1%})")
            except Exception:
                warnings.append(f"Overlap detection failed between layer '{profiles[orig_i]['layer']}' and '{profiles[j]['layer']}'")
    return warnings
