import re
import math
FOOTPRINT_HEIGHTS = {'USB_C_Receptacle': 3.5, 'USB_Micro': 3.0, 'USB_A': 8.0, 'HDMI': 6.0, 'RJ45': 14.0, 'Audio_Jack': 12.0, 'TerminalBlock': 12.0, 'MountingHole': 0.0, 'Battery': 5.0, 'QFN': 2.0, 'QFP': 3.5, 'BGA': 2.5, 'SOT23': 1.5, 'SOT223': 3.0, 'SOIC': 3.5, 'SO8': 3.0, 'D_PAK': 4.5, 'TO_220': 16.0, 'Button': 4.0, 'SW_Push': 4.0, 'LED': 2.0, 'Relay': 15.0, 'Fuse': 10.0, 'Diode': 2.0, 'Crystal': 2.0, 'Oscillator': 2.5}
...
SMD_HEIGHTS = {'0201': 0.3, '0402': 0.5, '0603': 0.8, '0805': 1.3, '1005': 3.5, '1206': 1.8, '1210': 2.5, '1806': 2.0, '1812': 2.5, '2012': 1.5, '2512': 2.0, '3216': 1.8}
...
def _parse_footprint_dimensions(block):
    pad_xs = []
    pad_ys = []
    for pad_m in re.finditer('\\(pad\\s+"[^"]*"\\s+\\S+\\s+\\S+\\s+\\(at\\s+([\\d.-]+)\\s+([\\d.-]+)', block):
...
def parse(filepath):
    ...
    return {'dimensions': dimensions, 'mounting_holes': holes, 'components': components, 'edge_connectors': connectors}
def _get_board_dimensions(content):
    xs, ys = ([], [])
    for m in re.finditer('\\(gr_line[^)]*\\(start\\s+([\\d.-]+)\\s+([\\d.-]+)\\)', content):
        chunk = content[m.start():m.start() + 1000]
        end_m = re.search('\\(end\\s+([\\d.-]+)\\s+([\\d.-]+)\\)', chunk)
        layer_m = re.search('\\(layer\\s+"?Edge\\.Cuts"?\\)', chunk)
        if end_m and layer_m:
            xs.extend([float(m.group(1)), float(end_m.group(1))])
            ys.extend([float(m.group(2)), float(end_m.group(2))])
    for m in re.finditer('\\(gr_arc[^)]*\\(start\\s+([\\d.-]+)\\s+([\\d.-]+)\\)', content):
        chunk = content[m.start():m.start() + 1000]
        end_m = re.search('\\(end\\s+([\\d.-]+)\\s+([\\d.-]+)\\)', chunk)
        layer_m = re.search('\\(layer\\s+"?Edge\\.Cuts"?\\)', chunk)
        if end_m and layer_m:
            xs.extend([float(m.group(1)), float(end_m.group(1))])
            ys.extend([float(m.group(2)), float(end_m.group(2))])
    for m in re.finditer('\\(gr_rect[^)]*\\(start\\s+([\\d.-]+)\\s+([\\d.-]+)\\)', content):
        chunk = content[m.start():m.start() + 1000]
        end_m = re.search('\\(end\\s+([\\d.-]+)\\s+([\\d.-]+)\\)', chunk)
        layer_m = re.search('\\(layer\\s+"?Edge\\.Cuts"?\\)', chunk)
        if end_m and layer_m:
    ...
    return {'width': round(max(x_vals) - min(x_vals), 2), 'height': round(max(y_vals) - min(y_vals), 2), 'x_min': round(min(x_vals), 2), 'y_min': round(min(y_vals), 2), 'x_max': round(max(x_vals), 2), 'y_max': round(max(y_vals), 2)}
def _get_mounting_holes(content):
    holes = []
    seen = set()
    fp_pat = '\\((?:footprint|module)\\s+(?:"([^"]*)"|([^\\s(")]+))'
    for m in re.finditer(fp_pat, content):
        name = m.group(1) or m.group(2)
        if 'MountingHole' not in name:
            continue
        block = _balanced_block(content, m.start())
        at_m = re.search('\\(at\\s+([\\d.-]+)\\s+([\\d.-]+)', block)
        if not at_m:
            continue
    ...
    for m in re.finditer(fp_pat, content):
        ...
        if (x, y) in seen:
            continue
        if re.search('\\(pad\\s+"[^"]*"\\s+(?:smd|thru_hole)\\s', block):
            continue
        pad_start = re.search('\\(pad\\s+"[^"]*"\\s+np_thru_hole\\s+circle', block)
        if not pad_start:
            continue
        pad_block = _balanced_block(block, pad_start.start())
        dm = re.search('\\(drill\\s+([\\d.-]+)\\)', pad_block)
        if dm:
...
def _extract_hole_diameter(name, chunk):
    m = re.search('MountingHole[_\\s]*([\\d.]+)', name)
    if m:
        return round(float(m.group(1)), 2)
    dm = re.search('\\(drill\\s+([\\d.-]+)\\)', chunk)
    if dm:
...
def _get_components(content, dimensions):
    ...
    for m in fp_iter:
        ...
        if 'MountingHole' in name:
            continue
        block = _balanced_block(content, m.start())
        at_m = re.search('\\(at\\s+([\\d.-]+)\\s+([\\d.-]+)(?:\\s+([\\d.-]+))?', block)
        if not at_m:
            continue
        x = round(float(at_m.group(1)), 2)
        y = round(float(at_m.group(2)), 2)
        ...
        comp = {'ref': ref, 'name': name, 'x': x, 'y': y, 'rotation': rotation, 'height': height, 'width': fpw, 'length': fpl, 'near_edge': False}
...
def _extract_reference(chunk):
    m = re.search('\\(fp_text\\s+reference\\s+"([^"]*)"', chunk)
    if m:
        return m.group(1)
    m = re.search('\\(property\\s+"Reference"\\s+"([^"]*)"', chunk)
    if m:
        return m.group(1)
    return '?'
...
if __name__ == '__main__':
    test_files = ['C:\\Users\\abdul\\Downloads\\PCB.kicad_pcb', 'C:\\Users\\abdul\\Documents\\PowerSupplyKit\\PowerSupplyKit.kicad_pcb']
