...
class PcbInputWidget(QtWidgets.QWidget):
    ...
    def _make_drop_page(self):
        ...
        self._drop_zone.setText("<div style='font-size:14px; color:#8b949e;'>DROP .kicad_pcb FILE HERE<br><span style='font-size:11px; color:#484f58;'>or click to browse</span></div>")
    ...
    def _browse_file(self):
        fp, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select KiCad PCB File', '', 'KiCad PCB (*.kicad_pcb)')
        if fp:
            self._load_board(fp)
    def _load_board(self, fp):
        ...
        if w == 100.0 and h == 60.0:
            self._status.setText('⚠ No Edge.Cuts found — using 100×60mm default. Check your KiCad file has a board outline.')
            self._status.setStyleSheet('color:#ff6b6b; font-size:11px;')
        else:
            self._status.setText(f'✓ {w}mm × {h}mm  |  {comps} components  |  {holes} mounting holes  |  {connectors} edge connectors')
            self._status.setStyleSheet('color:#22c55e; font-size:11px;')
        self._drop_zone.setText("<div style='font-size:11px; color:#8b949e; text-align:center;'>✓ PCB data loaded. Now use <b>File → Import → KiCad PCB</b> to see the 3D board.</div>")
        self._show_summary()
    ...
    def _make_summary_page(self):
        ...
        self._gen_btn.setToolTip('AI analyzes the PCB and generates a custom enclosure with connector cutouts')
    ...
    def _make_spin(self, min_v, max_v, default):
        ...
        s.setSingleStep(0.5)
        s.setSuffix(' mm')
        s.setStyleSheet('\n            QDoubleSpinBox {\n                background:\n                border: 1px solid\n                font-size: 11px; padding: 3px 6px;\n            }\n            QDoubleSpinBox:focus { border-color:\n        ')
        return s
    def _show_summary(self):
        ...
        try:
            ...
            hammond_fits = find_hammond_for_board(dims['width'], dims['height'], component_height=tallest)
        except Exception as ex:
        ...
        try:
            ...
            if _vstatus == VisionDeps.OK:
                ...
                _vdot = '<span style="color:#eab308;font-size:14px;">●</span> <span style="color:#8b949e;font-size:10px;" title="{}">Vision unavailable</span>'.format(_vmsg.replace('"', '&quot;'))
        except Exception:
            _vdot = ''
        summary = [f"<b style='color:#58a6ff;'>✓ {os.path.basename(self._board_path)} loaded</b> {_vdot}<br>", f"Board: {dims['width']} × {dims['height']} mm ({pcb_area:.0f}mm²)", f'Mounting holes: {len(holes)}', f'Edge connectors: {len(connectors)}']
        ...
        if hammond_fits:
            ...
            for mid, spec in hammond_fits[:5]:
                ...
                summary.append(f"<span style='color:#e6edf3;'>  {mid}  {ml}×{mw}×{mh}mm  (+{margin_w}W +{margin_h}H +{margin_z}Z margin)</span>")
        self._summary_label.setHtml('<br>'.join(summary))
        self.setCurrentIndex(1)
    def _on_generate(self):
        if not self._board_data:
            return
        params = {'wall_thickness': self._wall_t_spin.value(), 'lid_clearance': self._lid_clr_spin.value(), 'boss_od': self._boss_od_spin.value(), 'margin': 2.0, 'headroom_mm': 2.0, 'material': self._material_combo.currentText(), 'ai_mode': True}
