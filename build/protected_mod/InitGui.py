...
try:
    ...
    __file__ = _os.path.join(FreeCAD.getUserAppDataDir(), 'Mod', 'AICompanion', 'InitGui.py')
...
if _MISSING_DEPS:
    _msg = f"Missing critical dependencies: {', '.join(_MISSING_DEPS)}.\nRun this in your terminal to install:\n"
    if _os.name == 'nt':
        _msg += '  "C:\\Program Files\\FreeCAD 1.1\\bin\\python.exe" -m pip install -r "' + _os.path.join(_ADDON_DIR, 'requirements.txt') + '"\n'
    else:
...
_SUPPRESS_PATTERNS = ['constraintdesign', 'kicadstepup', 'kicad', 'stepup', 'no module named', 'failed to load', 'deprecated']
...
try:
    ...
    pass
import threading
def _restore(orig_warn=_ORIG_WARN, orig_log=_ORIG_LOG, ignore_cats=_IGNORE_CATS, _warnings=warnings, _FreeCAD_Console=FreeCAD.Console):
    try:
...
class AICompanionCommand:
    def GetResources(self):
        try:
            ...
            pixmap = ''
        return {'Pixmap': pixmap, 'MenuText': 'Open AI Copilot', 'ToolTip': 'Open AI design assistant (Ctrl+Shift+A)', 'Accel': 'Ctrl+Shift+A'}
    def Activated(self):
        try:
