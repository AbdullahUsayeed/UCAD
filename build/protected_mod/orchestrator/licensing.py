...
class LicenseManager:
    def __init__(self, config_path: str | None=None):
        self._config_path = config_path or os.path.join(os.path.dirname(os.path.dirname(__file__)), 'config.json')
    ...
    def _write_cache(self, data: dict) -> None:
        cache = {'valid': data.get('valid', False), 'activation_count': data.get('activation_count', 0), 'max_activations': data.get('max_activations', 3), 'expires_at': data.get('expires_at', ''), 'cached_at': time.time()}
