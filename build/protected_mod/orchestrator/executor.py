...
class StepResult:
    def __init__(self):
        ...
        self.raw_text = ''
class PlanExecutor:
    def __init__(self, generate_fn, execute_fn, observe_fn, diff_fn, extract_blocks_fn, build_messages_fn):
    ...
    def execute_step(self, prompt, plan_steps, step_idx, input_text, max_retries=5):
        ...
        for block in blocks:
            ...
            if not success and self.retries < max_retries:
                ...
                ctx = self.build_messages(input_text, mode='build', retry_context=f'Previous code failed: {message}. Current scene: {fresh_obs}')
...
FAILURE_MODES = {'syntax': ['SyntaxError', 'IndentationError', 'TabError', 'NameError', 'invalid syntax', 'unexpected indent', 'unmatched'], 'geometry': ['Part::', 'BRep', 'TopoDS', 'Shape', 'null shape', 'Failed to create', 'cannot compute', 'infinite'], 'execution': ['TypeError', 'ValueError', 'AttributeError', 'KeyError', 'IndexError', 'ZeroDivisionError', 'RuntimeError', "'NoneType'", 'is not defined', "module 'Part'"], 'api': ['FreeCAD', 'App::', 'Gui::', 'Document', 'Object', 'Property', 'no active document', 'is not attached', 'not found'], 'timeout': ['timeout', 'timed out', 'Timeout', 'TimeoutError'], 'memory': ['MemoryError', 'out of memory', 'Kernel'], 'ai_format': ['exceeded maximum context', 'rate_limit', 'content_filter', 'API key', '401', '403', '429', 'insufficient_quota']}
