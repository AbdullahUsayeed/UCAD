"""API Key management dialog — provider selection, API key, Ollama URL, Test Connection."""
from compat import QtWidgets, QtCore
from orchestrator import (
    PROVIDERS, PROVIDER_HELP_URLS, LITELLM_PROVIDERS, PROVIDER_ADAPTERS,
)
import json, urllib.request, urllib.error, ssl


def _check_host_reachable(host, port=443, timeout=5):
    """Quick TCP connectivity check — returns (ok, detail)."""
    import socket
    try:
...
def show_ai_setup_dialog(sidebar):
    ...
    d.setStyleSheet("""
        ...
        QPushButton.test_ok { background:#1a3a2a; color:#22c55e; }
        QPushButton.test_fail { background:#3a1a1a; color:#ff6b6b; }
        QFrame.separator { color:#2e3e56; }
    """)
    ...
    std_warning = QtWidgets.QLabel(
        "\u26a0\ufe0f <b>Limited capabilities:</b> Local models lack specialized guides "
        "(gears, airfoils, etc.), detailed scene context, conversation memory, "
        "and auto-retry &mdash; keep prompts precise and simple."
    )
        ...
        if provider != "ollama":
            if sidebar._provider_requires_key(provider) and not key:
                QtWidgets.QMessageBox.warning(
                    d, "API Key Required",
                    f"Please enter an API key for {sidebar._pretty_provider(provider)}."
                )
                return
        else:
