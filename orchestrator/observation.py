"""Scene observation: diff-aware scene formatting and prompt composition."""


class SmartObserver:
    """Diff-aware, token-budget-aware scene formatter."""
    ...
    def diff_summary(self, max_chars: int = 600) -> str:
        ...
        modified = [
            curr_map[uid] for uid in curr_map
            if uid in prev_map
            and prev_map[uid].get("shape_hash") != curr_map[uid].get("shape_hash")
        ]
...
class PromptComposer:
    """Assembles system and user prompts with strict token discipline."""
    ...
    def system_prompt(self, mode: str, kb_text: str,
                      api_corrections: str = "",
                      failure_lessons: str = "") -> str:
    ...
    def user_prompt(self, user_input: str, mode: str,
                    scene_summary: str = "",
                    selection_context: str = "",
                    history_entries: list = None,
                    dependency_chain: str = "",
                    relevant_objects: str = "",
                    viewport_text: str = "") -> str:
        sections = []
        if scene_summary:
    ...
    def _role_instruction(mode: str) -> str:
        instructions = {
            "build": (
                "You are an autonomous FreeCAD design agent.\n"
            ...
            "ask": "You are a FreeCAD assistant. Answer concisely. No code unless asked.",
        }
