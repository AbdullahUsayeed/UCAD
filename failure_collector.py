"""Structured failure collector.

Replaces all bare `except: pass` with routed failure records.
Top failures are injected into the system prompt as "LESSONS LEARNED",
creating a self-improvement loop within each session.
"""
...
class FailureCollector:
    ...
    def as_prompt_section(self):
        ...
        for f in failures:
            lines.append(
                f"- {f['source']}: {f['type']} (x{f['count']}) — {f['message']}"
            )
        return "\n".join(lines)
