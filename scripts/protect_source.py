"""Strip comments, docstrings, and excess whitespace from .py files.

Outputs obfuscated copies that are functionally identical but much
harder to read.  Works across all Python versions.
"""
import ast
import re
import shutil
...
EXCLUDE_FOLDERS = {
    "build", "dist", "__pycache__", ".git", ".pytest_cache",
...
def _strip_comments(text):
    """Remove single-line comments (# ...) preserving string contents."""
    lines = []
...
def _strip_docstrings(text):
    """Remove docstrings from modules, functions, classes via AST."""
    try:
        tree = ast.parse(text)
...
def main():
    ...
    # Remove __init__ stubs (overwrite with originals)
